...
export default class SimpleReport extends Component {
	...
	render() {
		const { simpleReportXML, simpleReportHTML } = this.props.reports;
		const { iframeHeight, rawReportExpanded, rawReportBeauty } = this.state;
		return (
			<div className="w-100">
				<iframe frameBorder="0" className="w-100 mb-2" style={{ height: iframeHeight }} srcDoc={simpleReportHTML} title="SimpleReport"></iframe>
				{iframeHeight ? (
					<Card elevation={3} className="mb-3 card">
						...
						<Collapse in={rawReportExpanded} timeout={500}>
							<CardContent className="overflow-auto" style={{ maxHeight: "calc(100vh - 91.2px - 64px - 58px - 20px)" }}>
								{rawReportBeauty ? <XMLViewer xml={simpleReportXML} indentSize={4} collapsible /> : <pre>{simpleReportXML}</pre>}
							</CardContent>
						</Collapse>
					</Card>
				) : null}
